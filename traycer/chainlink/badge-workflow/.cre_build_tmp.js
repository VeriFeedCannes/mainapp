...
var onCronTrigger = (runtime2) => {
  const { apiBaseUrl, consumerAddress, chainName, gasLimit, creWebhookSecret } = runtime2.config;
  runtime2.log("Fetching next mintable claim...");
  const claim = runtime2.runInNodeMode(fetchNextClaim, consensusIdenticalAggregation())(apiBaseUrl).result();
  if (!claim.hasClaim || !claim.eligible || claim.badgeId === 0) {
    runtime2.log("No pending claim — skipping");
    return { eligible: false, badgeId: 0n, txHash: "" };
  }
  runtime2.log(`Claim ${claim.claimId}: wallet=${claim.wallet} badgeId=${claim.badgeId} returns=${claim.totalReturns}`);
  ...
  const reportData = encodeAbiParameters(parseAbiParameters("address wallet, uint256 badgeId, uint256 totalReturns"), [
    claim.wallet,
  ...
  runtime2.log(`Writing report to ${consumerAddress} on World Chain...`);
  ...
  runtime2.log(`Badge #${claim.badgeId} minted! tx: ${txHash}`);
  runtime2.log(`https://worldscan.org/tx/${txHash}`);
  runtime2.log("Confirming mint on backend...");
  runtime2.runInNodeMode(confirmMint, consensusIdenticalAggregation())(apiBaseUrl, claim.claimId, txHash, creWebhookSecret).result();
  runtime2.log(`Claim ${claim.claimId} confirmed as minted`);
  return {
    eligible: true,
...
var fetchNextClaim = (nodeRuntime, apiBaseUrl) => {
...
var confirmMint = (nodeRuntime, apiBaseUrl, claimId, txHash, creWebhookSecret) => {
  const httpClient = new ClientCapability2;
